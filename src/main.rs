...
use commands::{
    ...
    normal_message, unknown_command, wha_happun,
};
use log::{error, info};
use serenity::{
    async_trait,
    ...
    async fn interaction_create(&self, ctx: Context, interaction: Interaction) {
        if let Interaction::ApplicationCommand(command) = interaction {
            let content = match command.data.name.as_str() {
                ...
                "remove" => match (command.guild_id, command.data.options.get(0)) {
                    (Some(id), Some(opts)) => {
                        if let Some(ApplicationCommandInteractionDataOptionValue::String(v)) =
                            &opts.resolved
                        {
                ...
                "news" => match command.data.options.get(0) {
                    Some(opts) => {
                        if let Some(ApplicationCommandInteractionDataOptionValue::Integer(i)) =
                            &opts.resolved
                        {
                            match wha_happun::wha_happun(Some(*i)).await {
                                Ok(t) => t,
                                Err(e) => e.to_string(),
                            }
                        } else {
                    ...
                    None => match wha_happun::wha_happun(None).await {
                        Ok(t) => t,
                        Err(e) => e.to_string(),
                    },
                },
    ...
    async fn ready(&self, ctx: Context, ready: Ready) {
        ...
        let commands = ApplicationCommand::set_global_application_commands(&ctx.http, |commands| {
            commands
                ...
                .create_application_command(|command| {
                    command.name("remove").description("Remove a campaign").create_option(|option| {
                        option.name("campaign").description("The campaign to remove").kind(ApplicationCommandOptionType::String).required(true)
                    })
                })
                .create_application_command(|command| {
                    command.name("news").description("What's new in trashbot").create_option(|option| {
                        option.name("count").description("How many changes to show (deafult 5)").kind(ApplicationCommandOptionType::Integer)
                    })
                })
        })
        .await;
